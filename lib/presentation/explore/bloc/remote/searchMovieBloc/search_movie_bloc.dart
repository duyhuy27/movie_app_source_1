...
class SearchMovieBloc extends Bloc<SearchMovieEvent, SearchMovieState> {
  ...
  SearchMovieBloc(this.searchMovieUseCase) : super(const SearchMovieState()) {
    on<GetMovieBySearchEvent>(_getMovieBySearch,
        transformer: debounce(const Duration(milliseconds: 1000)));

    on<FilteredGetMovieBySearchEvent>(_getFilteredMovieBySearch);
  }

  Future<void> _getFilteredMovieBySearch(
      FilteredGetMovieBySearchEvent event, Emitter<SearchMovieState> emit) async {
    emit(state.copyWith(
  ...
  Future<void> _getMovieBySearch(
      GetMovieBySearchEvent event, Emitter<SearchMovieState> emit) async {
