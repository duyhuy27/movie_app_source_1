...
import 'package:huynd2_assignment/presentation/see_all_trending/bloc/trending_movie/trending_movie_state.dart';
...
class TrendingMovieBloc extends Bloc<TrendingMovieEvent, TrendingMovieState> {
  ...
  Future<void> _getTrendingMovies(
      GetTrendingMoviesEvent event, Emitter<TrendingMovieState> emit) async {
    emit(state.copyWith(
  ...
  Future<void> _loadMoreTrendingMovies(LoadMoreTrendingMoviesEvent event,
      Emitter<TrendingMovieState> emit) async {
    emit(state.copyWith(isLoadMore: true, status: Status.loading));
    try {
      final nextPage = state.currentPage + 1;
      final dataState = await movieUseCase.getTrendingMovieUseCase(
          event.timeWindow, nextPage);
      final updatedMovies = List<MovieEntity>.from(state.trendingMovies!)
        ..addAll(dataState.results ?? []);
      emit(state.copyWith(
          status: Status.success,
  ...
  Future<void> _filterTrendingMovies(
      FilterTrendingMoviesEvent event, Emitter<TrendingMovieState> emit) async {
    emit(state.copyWith(
        isRefresh: true,
        status: Status.loading,
    ...
    try {
      final dataState = await movieUseCase.getTrendingMovieUseCase(
          event.timeWindow, 1); // Replace with your time window logic
      final filteredMovies =
          _filterMoviesByGenre(dataState.results, event.genreId);
  ...
  List<MovieEntity>? _filterMoviesByGenre(
      List<MovieEntity>? movies, int? genreId) {
    return genreId != null
        ? movies
