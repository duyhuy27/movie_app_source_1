...
class FavouriteBloc extends Bloc<FavouriteEvent, FavouriteState> {
  ...
  Future<void> _resetFavourite(
      ResetFavourite event, Emitter<FavouriteState> emit) async {
    emit(state.copyWith(status: Status.initial, favourites: []));
  }

  Future<void> _onAddFavourite(
      AddFavourite event, Emitter<FavouriteState> emit) async {
  ...
  Future<void> _getFavourite(
      GetFavourites event, Emitter<FavouriteState> emit) async {
    emit(state.copyWith(status: Status.loading));
    try {
      final favourites =
          await favouriteUseCase.getFavouriteUseCase(event.userId);
  ...
  Future<void> _deleteFavourite(
      RemoveFavourite event, Emitter<FavouriteState> emit) async {
    final loadingItemId = event.movieId;
    emit(state.copyWith(loadingItemId: loadingItemId));
    try {
      await favouriteUseCase.deleteFavouriteUseCase(
          event.userId, event.movieId);
      final updatedFavourites = List<UserFavouriteEntity>.from(state.favourites)
      ...
      emit(state.copyWith(
          status: Status.failure, message: e.message, loadingItemId: null));
    }
  }
}
