...
import 'package:huynd2_assignment/presentation/see_all_top_rated/bloc/top_rated/top_rated_movie_state.dart';
...
class TopRatedMovieBloc extends Bloc<TopRatedMovieEvent, TopRatedMovieState> {
  ...
  Future<void> _getTopRatedMovies(
      GetTopRatedMovieEvent event, Emitter<TopRatedMovieState> emit) async {
    emit(state.copyWith(
  ...
  Future<void> _loadMoreTopRatedMovies(LoadMoreTopRatedMovieEvent event,
    ...
    try {
      ...
      final updatedMovies = List<MovieEntity>.from(state.moviesTopRated!)
  ...
  void _updateFilter(
      UpdateFilterEvent event, Emitter<TopRatedMovieState> emit) {
    final filteredMovies = state.moviesTopRated!
        .where((movie) => movie.voteAverage! >= event.minVoteAverage)
        .toList();
    emit(state.copyWith(
      status: Status.success,
