...
import 'package:huynd2_assignment/presentation/see_all_new_movie/bloc/new_movie/new_movie_state.dart';
...
class NewMovieBloc extends Bloc<NewMovieEvent, NewMovieState> {
  ...
  Future<void> _getNewMovies(
      GetNewMoviesEvent event, Emitter<NewMovieState> emit) async {
    emit(state.copyWith(
        isRefresh: event.isRefresh, status: Status.loading, currentPage: 1));
    try {
  ...
  Future<void> _loadMoreNewMovies(
      LoadMoreNewMoviesEvent event, Emitter<NewMovieState> emit) async {
    ...
    try {
      ...
      final updatedMovies = List<MovieEntity>.from(state.newMovies!)
  ...
  Future<void> _filterNewMovies(
      FilterNewMoviesEvent event, Emitter<NewMovieState> emit) async {
    emit(
        state.copyWith(status: Status.loading, selectedGenreId: event.genreId));
    try {
      final dataState = await movieUseCase.getUpComingMovieUseCase(1);
