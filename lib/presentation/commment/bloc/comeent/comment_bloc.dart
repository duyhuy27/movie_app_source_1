...
class CommentBloc extends Bloc<CommentEvent, CommentState> {
  ...
  void _onRefreshCommentRequested(
      RefreshCommentRequested event, Emitter<CommentState> emit) async {
    ...
    try {
      ...
      emit(state.copyWith(
          status: CommentStatus.error, errorMessage: e.toString()));
    }
  }

  void _onAddCommentRequested(
      AddCommentRequested event, Emitter<CommentState> emit) async {
    try {
      await commentUseCase.addComment(event.comment);
      final comments =
          await commentUseCase.getCommentsUseCase(event.comment.postId ?? '');
      emit(state.copyWith(
  ...
  void _onDeleteCommentRequested(
  ...
  void _onGetCommentsRequested(
      GetCommentsRequested event, Emitter<CommentState> emit) async {
    emit(state.copyWith(status: CommentStatus.loading));
    try {
      final comments = await commentUseCase.getCommentsUseCase(event.postId);
      emit(state.copyWith(status: CommentStatus.success, comments: comments, countComments: comments.length));
  ...
  void _onGetRepliesRequested(
      GetRepliesRequested event, Emitter<CommentState> emit) async {
    emit(state.copyWith(status: CommentStatus.loading));
    try {
  ...
  void _onUpdateCommentRequested(
      UpdateCommentRequested event, Emitter<CommentState> emit) async {
    emit(state.copyWith(status: CommentStatus.loading));
    try {
      await commentUseCase.updateComment(event.comment);
      final comments = await commentUseCase.getCommentsUseCase(event.comment.postId ?? '');
      emit(state.copyWith(status: CommentStatus.success, comments: comments, replyingTo: null, countComments: comments.length));
    } catch (e) {
      emit(state.copyWith(
          status: CommentStatus.error, errorMessage: e.toString()));
    }
  ...
  void _onReplyToCommentRequested(
      ReplyToCommentRequested event, Emitter<CommentState> emit) {
