...
import 'package:huynd2_assignment/manager/widget/cached_image_custom_widget.dart';
import 'package:huynd2_assignment/manager/widget/custom_spin_widget.dart';
import 'package:huynd2_assignment/presentation/downloaded_video/screen/see_download_video.dart';
...
import 'package:huynd2_assignment/presentation/trailer/widget/dropdown_type_widget.dart';
import 'package:huynd2_assignment/presentation/trailer/widget/trailer_section_widget.dart';
import 'package:huynd2_assignment/presentation/trailer/widget/youtube_trailer_widget.dart';
      ...
      body: BlocBuilder<TrailerBloc, TrailerState>(
        builder: (context, state) {
          if (state.status == Status.loading) {
            ...
            if (state.trailers?.isNotEmpty == true &&
                state.trailers!.first.key != null) {
              if (!state.isControllerReady) {
                _controller.load(state.trailers!.first.key!);
                context.read<TrailerBloc>().add(
                    const UpdateControllerReadyEvent(isControllerReady: true));
              }
                ...
                slivers: [
                  SliverAppBar(
                    ...
                    flexibleSpace: Stack(
                      fit: StackFit.expand,
                      children: <Widget>[
                        CachedImageCustomWidget(
                            widget: widget, imageUrl: movie.backdropPath ?? ''),
                        Align(
                        ...
                        children: [
                          DropDownTypeWidget(
                            ...
                            onChanged: (String? value) {
                              context.read<TrailerBloc>().add(
                                  UpdateTrailerTypeEvent(trailerType: value!));
                            },
                          ),
                          16.verticalSpace,
