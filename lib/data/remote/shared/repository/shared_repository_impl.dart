...
class SharedRepositoryImpl extends SharedRepository {
  ...
  @override
  Future<SharedResponse> updateShared(
      SharedEntity sharedEntity, String id) async {
    try {
  ...
  Future<List<SharedResponse>> _getSharedCachedOrApi(
      int page,
      String type,
      Future<List<SharedResponse>> Function() apiCall) async {
    try {
      final cachedData = await _databaseHelper.getFeed(page, type);

      if (cachedData != null) {
        final List<SharedResponse> shared = (json.decode(cachedData) as List)
            .map((i) => SharedResponse.fromJson(i))
            .toList();
        if (kDebugMode) {
      ...
      final data =
          json.encode(httpResponse.map((e) => e.toJson()).toList());
      if (kDebugMode) {
        print('data: $data');
      } // Debug log
