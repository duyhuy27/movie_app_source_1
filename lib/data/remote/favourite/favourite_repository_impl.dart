...
class FavouriteRepositoryImpl extends FavouriteRepository {
  ...
  Future<void> addFavourite(UserFavouriteEntity favourite, String userId) async {
    try {
      final docRef =
          firestore.collection('users').doc(userId).collection('favourites').doc('${favourite.id}'); 
  ...
  Future<List<UserFavouriteEntity>> getFavourites(String uid) async {
    try {
      final querySnapshot =
          await firestore.collection('users').doc(uid).collection('favourites').get();
  ...
  Future<void> removeFavourite(String userId, int movieId) async {
    try {
      final docRef =
          firestore.collection('users').doc(userId).collection('favourites').doc('$movieId');
      await docRef.delete();
    } catch (e) {
