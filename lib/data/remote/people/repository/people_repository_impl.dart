...
class PeopleRepositoryImpl implements PeopleRepository {
  ...
  Future<PageResponse<List<ActorResponse>>> getPopularPeople(
      {int page = 1}) async {
    try {
      return await _getPeopleFromCacheOrApi(page, 'popular_people',
        () => _peopleApiService.getPopularPeople(page: page));
    }
    catch (e) {
  ...
  Future<PageResponse<List<ActorResponse>>> _getPeopleFromCacheOrApi(
      int page,
      String type,
      Future<PageResponse<List<ActorResponse>>> Function() apiCall) async {
    try {
      final cachedData = await _databaseHelper.getMovie(page, type);

      if (cachedData != null) {
        final List<ActorResponse> actors = (json.decode(cachedData) as List)
            .map((i) => ActorResponse.fromJson(i))
            .toList();
        if (kDebugMode) {
