...
class TVShowRepositoryImpl extends TVShowRepository {
  ...
  Future<PageResponse<List<TVShowResponse>>> getTvShowAirToDay() async {
    return await _getTvShowFromCacheOrApi(
        1, "airing_today", () => _tvShowApiService.getAiringTodayTVShows(page: 1));
  }

  @override
  Future<PageResponse<List<TVShowResponse>>> getTvShowPopular() async {
    return await _getTvShowFromCacheOrApi(
        1, "popular", () => _tvShowApiService.getPopularTVShows(page: 1));
  }
  ...
  Future<PageResponse<List<TVShowResponse>>> _getTvShowFromCacheOrApi(
      int page,
      String type,
      Future<PageResponse<List<TVShowResponse>>> Function() apiCall) async {
    try {
      final cachedData = await _databaseTvShowCachedHelper.getTvShowCached(page, type);

      if (cachedData != null) {
        final List<TVShowResponse> actors = (json.decode(cachedData) as List)
            .map((i) => TVShowResponse.fromJson(i))
            .toList();
        if (kDebugMode) {
